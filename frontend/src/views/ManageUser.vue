        ...
        <div class="card-body">
          ...
          <div v-if="userData && userData.id" class="grid grid-cols-2 gap-8">
            <div>
              ...
              <p class="my-2">
                <span class="pr-2 font-semibold">Perfil:</span>
                <span>
                  {{ userData.role }}
                </span>
              </p>
...
const handleChangeRole = async () => {
  const newRole = userData.value.role === "user" ? "admin" : "user";

  userData.value = await usersStore.updateUser(userData.value.id, {
    role: newRole,
  });
};
